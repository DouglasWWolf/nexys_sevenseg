...
# module references:
# axi_7seg, sevenseg_driver, axi_uart_bridge
...
proc create_root_design { parentCell } {
  ...
  # Create instance: axi_7seg, and set properties
  set block_name axi_7seg
  set block_cell_name axi_7seg
  if { [catch {set axi_7seg [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $axi_7seg eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
  ...
  connect_bd_intf_net -intf_net system_interconnect_M00_AXI [get_bd_intf_pins axi_7seg/S_AXI] [get_bd_intf_pins system_interconnect/M00_AXI]
  ...
  connect_bd_net -net axi_7seg_digit_enable [get_bd_pins axi_7seg/digit_enable] [get_bd_pins sevenseg_driver/digit_enable]
  connect_bd_net -net axi_7seg_display [get_bd_pins axi_7seg/display] [get_bd_pins sevenseg_driver/display]
  ...
  connect_bd_net -net source_100mhz_peripheral_aresetn [get_bd_pins axi_7seg/resetn] [get_bd_pins axi_uart_bridge/aresetn] [get_bd_pins sevenseg_driver/resetn] [get_bd_pins source_100mhz/peripheral_aresetn]
  connect_bd_net -net system_clock_clk_100mhz [get_bd_pins axi_7seg/clk] [get_bd_pins axi_uart_bridge/aclk] [get_bd_pins sevenseg_driver/clk] [get_bd_pins source_100mhz/clk_100mhz] [get_bd_pins system_interconnect/aclk]

  # Create address segments
  assign_bd_address -offset 0x00001000 -range 0x00000080 -target_address_space [get_bd_addr_spaces axi_uart_bridge/axi_uart_bridge/M_AXI] [get_bd_addr_segs axi_7seg/S_AXI/reg0] -force
